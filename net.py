...
class Generator1(nn.Module):
    ...
    def __init__(self, in_ch, n_spk, z_ch, mid_ch, s_ch, normtype='IN', src_conditioning=False):
        ...
        self.le10 = nn.Conv1d(mid_ch + s_ch, in_ch, 9, stride=1, padding=(9 + 1) // 2 + 1)
...
class StarGAN(nn.Module):
    ...
    def calc_advloss_g(self, df_adv_ss, df_adv_st, df_adv_tt, df_adv_ts):
        ...
        if self.loss_type == 'wgan':
            ...
            AdvLoss_g = (
                                F.binary_cross_entropy_with_logits(df_adv_ss, torch.ones_like(df_adv_ss),
                                                                   reduction='sum') +
                                F.binary_cross_entropy_with_logits(df_adv_st, torch.ones_like(df_adv_st),
                                                                   reduction='sum') +
                                F.binary_cross_entropy_with_logits(df_adv_tt, torch.ones_like(df_adv_tt),
                                                                   reduction='sum') +
                                F.binary_cross_entropy_with_logits(df_adv_ts, torch.ones_like(df_adv_ts),
                                                                   reduction='sum')
                        ) / (df_adv_ss.numel() + df_adv_st.numel() + df_adv_tt.numel() + df_adv_ts.numel())
    ...
    def calc_advloss_d(self, x_s, x_t, xf_ts, xf_st, dr_adv_s, dr_adv_t, df_adv_ss, df_adv_st, df_adv_tt, df_adv_ts):
        ...
        if self.loss_type == 'wgan':
            ...
            AdvLoss_d_r = (
                                  F.binary_cross_entropy_with_logits(dr_adv_s, torch.ones_like(dr_adv_s),
                                                                     reduction='sum') +
                                  F.binary_cross_entropy_with_logits(dr_adv_t, torch.ones_like(dr_adv_t),
                                                                     reduction='sum')
                          ) / (dr_adv_s.numel() + dr_adv_t.numel())
            AdvLoss_d_f = (
                                  F.binary_cross_entropy_with_logits(df_adv_ss, torch.zeros_like(df_adv_ss),
                                                                     reduction='sum') +
                                  F.binary_cross_entropy_with_logits(df_adv_st, torch.zeros_like(df_adv_st),
                                                                     reduction='sum') +
                                  F.binary_cross_entropy_with_logits(df_adv_tt, torch.zeros_like(df_adv_tt),
                                                                     reduction='sum') +
                                  F.binary_cross_entropy_with_logits(df_adv_ts, torch.zeros_like(df_adv_ts),
                                                                     reduction='sum')
                          ) / (df_adv_ss.numel() + df_adv_st.numel() + df_adv_tt.numel() + df_adv_ts.numel())
            AdvLoss_d = AdvLoss_d_r + AdvLoss_d_f
    ...
    def calc_gen_loss(self, x_s, x_t, k_s, k_t, mel_masks):
        # Generator outputs
        ...
        # import pdb
        # pdb.set_trace()
        xf_ss = self.gen(x_s, k_s, k_s)[:, :, :time_dim]
        xf_ts = self.gen(x_t, k_s, k_t)[:, :, :time_dim]
        ...
        xf_st = xf_st.masked_fill(mel_masks, 0.)

        # print(f"x_s: {x_s.shape}")
        # print(f"x_t: {x_t.shape}")
        # print(f"xf_ss: {xf_ss.shape}")
        # print(f"xf_ts: {xf_ts.shape}")
        # print(f"xf_tt: {xf_tt.shape}")
        # print(f"xf_st: {xf_st.shape}")
        ...
        masked_xf_st_ts = self.gen(xf_st, k_s, k_t)[:, :, :time_dim].masked_fill(mel_masks, 0.)
        masked_xf_ts_st = self.gen(xf_ts, k_t, k_s)[:, :, :time_dim].masked_fill(mel_masks, 0.)
        CycLoss = (
                          torch.sum(torch.abs(x_s - masked_xf_st_ts)) +
                          torch.sum(torch.abs(x_t - masked_xf_ts_st))
                  ) / (x_s.numel() + x_t.numel())
    ...
    def calc_dis_loss(self, x_s, x_t, k_s, k_t, mel_masks):
        device = x_s.device
        time_dim = x_s.shape[-1]
        n_mels = x_s.shape[1]
        ...
        # Adversarial loss
        AdvLoss_d, GradLoss_d = self.calc_advloss_d(x_s, x_t, xf_ts, xf_st, dr_adv_s, dr_adv_t, df_adv_ss, df_adv_st,
                                                    df_adv_tt, df_adv_ts)
